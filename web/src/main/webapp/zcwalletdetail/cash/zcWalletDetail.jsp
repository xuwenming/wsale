...
<c:if test="${fn:contains(sessionInfo.resourceList, '/zcWalletDetailController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/zcWalletDetailController/viewByCash')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/zcWalletDetailController/cashDataGrid?wtype=WT02',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				title : '<%=TzcWalletDetail.ALIAS_HANDLE_USER_ID%>',
				width : 50		
				}, {
				field : 'handleRemark',
				title : '<%=TzcWalletDetail.ALIAS_HANDLE_REMARK%>',
				width : 80
				}, {
				field : 'handleTime',
				title : '<%=TzcWalletDetail.ALIAS_HANDLE_TIME%>',
				width : 80,
				sortable:true
			}, {
				field : 'action',
				...
				formatter : function(value, row, index) {
					var str = '';
					if ($.canEdit && row.wtype == 'WT02' && (row.handleStatus == 'HS01' || row.handleStatus == 'HS02')) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			buttons : [ {
				...
				text : '不同意',
				handler : function() {
					parent.$.messager.confirm('询问', '不同意提现，是否继续？', function(b) {
						if (b) {
							parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
							var f = parent.$.modalDialog.handler.find('#form');
							f.find("input[name=handleStatus]").val("HS04");
							f.submit();
						}
					});
				}
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/zcWalletDetailController/viewByCash?id=' + id
		});
	}
...
<body>
	...
	<div id="toolbar" style="display: none;">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/zcWalletDetailController/download')}">
