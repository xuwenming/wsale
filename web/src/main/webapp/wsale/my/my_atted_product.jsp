...
<body>
    <div data-role="page" data-title="我的关注" class="jqm-demos">
        <div role="main" class="ui-content jqm-content jqm-fullwidth">
            ...
            <div class="atted-product-list" style="margin-top: 10px;">
    ...
    <script type="text/javascript">
        ...
        function drawProductDetail() {
            ajaxPost('api/apiProductController/productAllAttredList', {page:pCurrPage, rows:pRows}, function(data){
                if(data.success) {
                    var result = data.obj;
                    if(result.rows.length != 0) {
                        ...
                        if($('.atted-product-list').children().length == 0)
                            $(".atted-product-list").append(Util.noDate(2, '没有相关的拍品，关注更多集友哦！'));
                    }
        ...
        function buildProductDetail(productDetail) {
            var product = productDetail.product, user = productDetail.user;
            var viewData = Util.cloneJson(productDetail);
            viewData.user.positionIcon = base + 'wsale/images/p-'+user.positionId+'.png';
        ...
        function createEvent(dom, productDetail) {
            ...
            dom.find('.auction_btn').click({product:productDetail.product, self:productDetail.user.self}, auction);
            if($.authAutoBid) {
                dom.find(".auto_auction_btn").click({product:productDetail.product, self:productDetail.user.self}, function(event){
                    if('${subscribe}' == 0) {
                        $('.mask-layer, .subscribe').show();
                        addSubscribeLog(event.data.product.id);
                        return;
                    }
                    ...
                    var marginFlag = product.marginFlag;
                        ...
                        $.modal({
                            ...
                            buttons: [
                                ...
                                { text: "去支付", onClick: function(){
                                    ajaxPost('api/apiProductController/addMargin', {productId: product.id, margin: margin}, function(data){
                                        if(data.success) {
                    ...
                    $('#maxPricePopup').attr({'data-product-id':product.id, 'data-product-fixed' : product.fixedPrice, 'data-bid' : bid});
            ...
            dom.find('.ppxq-touxiang').click(productDetail.user.id, function(event){
            ...
            dom.find('.share-icon').click({productId:productDetail.product.id, nickname:productDetail.user.nickname}, function(event){
                $('.share-list li:eq(1) .msg').text('发送链接');
                $('.share-list li:eq(2) .msg').text('用微信分享');
                $('.share-list').attr({'data-product-id':event.data.productId});
                $('#sharePopup').wePopup();
            });
        ...
        function auction(event) {
            var product = event.data.product, self = event.data.self;
            if('${subscribe}' == 0) {
                $('.mask-layer, .subscribe').show();
...
//                elm.append('<img class="lazy ppxq-imglist" data-original="'+files[i].fileHandleUrl+'" />\n');
        ...
        function drawAuction(elm, product, load) {
            ...
            ajaxPostSync('api/apiProductController/auctions', {productId:product.id, page:currPage, rows:rows}, function(data){
                if(data.success) {
                    ...
                    var flag = product.addUserId != '${sessionInfo.id}';
                    if(result.rows.length != 0) {
                        for(var i in result.rows) {
        ...
        function buildAuction(elm, auction, flag) {
            var isClick = true;
            if('<%=amSwitch %>' == 1 && flag && auction.user.id != '${sessionInfo.id}') {
