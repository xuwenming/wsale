...
<script type="text/javascript">
	...
	function createWxMenu() {
		parent.$.messager.confirm('询问', '您是否要刷新微信自定义菜单？', function(b) {
			if (b) {
				parent.$.messager.progress({
					title : '提示',
					text : '数据处理中，请稍后....'
				...
				$.getJSON('${pageContext.request.contextPath}/resourceController/createWxMenu', {
				}, function(result) {
					parent.$.messager.progress('close');
					if (result.success) {
						parent.$.messager.alert('提示', result.msg, 'info');
...
<body>
	...
	<div id="toolbar" style="display: none;">
		...
		<c:if
				test="${fn:contains(sessionInfo.resourceList, '/resourceController/createWxMenu')}">
			<a onclick="createWxMenu();" href="javascript:void(0);"
			   class="easyui-linkbutton"
			   data-options="plain:true,iconCls:'exclamation'">刷新微信菜单</a>
		</c:if>
	</div>
