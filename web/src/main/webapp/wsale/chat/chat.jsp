    ...
    <script type="text/javascript">
        ...
        function buildMessage(message, append) {
            ...
            if(mtype == 'TEXT') {
                ...
                viewData.content = '<div class="productMsg" data-product="'+message.product.id+'"><div data-original="'+message.product.icon+'" style="width: 180px;height:0;padding-bottom:100px; background-position:center center;background-size:cover;" class="imageMsg lazy"></div><div><a style="float:left;">拍品：'+message.product.content+'</a></div></div>';
            } else if(mtype == 'SHOWPRODUCT') {
                viewData.content = '<div class="productMsg" data-product="${product.id}"><div data-original="${product.icon}" style="width: 180px;height:0;padding-bottom:100px; background-position:center center;background-size:cover;" class="imageMsg lazy"></div><div><a href="#" style="max-height: 40px;overflow-y: hidden;">拍品：${fn:replace(product.content, vEnter, '')}</a><div class="sendProduct" style="float:right;padding-top:5px;">发送拍品</div></div></div>';
            } else if(mtype == 'BBS') {
                viewData.content = '<div class="bbsMsg" data-bbs="'+message.bbs.id+'"><div data-original="'+message.bbs.icon+'" style="width: 180px;height:0;padding-bottom:100px; background-position:center center;background-size:cover;" class="imageMsg lazy"></div><div><a style="float:left;">帖子：'+message.bbs.bbsTitle+'</a></div></div>';
            } else if(mtype == 'SHOWBBS') {
                viewData.content = '<div class="bbsMsg" data-bbs="${bbs.id}"><div data-original="${bbs.icon}" style="width: 180px;height:0;padding-bottom:100px; background-position:center center;background-size:cover"; class="imageMsg lazy"></div><div><a href="#" style="max-height: 40px;overflow-y: hidden;">帖子：${fn:replace(bbs.bbsTitle, vEnter, '')}</a><div class="sendBbs" style="float:right;padding-top:5px;">发送帖子</div></div></div>';
            }
